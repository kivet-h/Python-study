...
# print([x + x for x in range(5)]) # ==> [0, 2, 4, 6, 8]
...
# print(val) # ==> 打印出当前文件同级目录下所有文件名称，list形式，如：['7.1 切片.py', '7.2 迭代.py', '7.3 列表生成式.py']


# 前面迭代有介绍，for循环是可以同时使用两个甚至多个变量，如dict的items()可以同时迭代key和value
# d = {'a': 1, 'b': 2, 'c': 3}
# for k, v in d.items():
#     print(k, '=', v)
# # ==>
...
# 总结：可见，在一个列表生成式中，for前面的 if...else 是表达式，而for后面的if是过滤条件，不能带else。
