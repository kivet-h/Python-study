...
# 类名后紧接着是(object)，表示该类是从哪个类继承下来的。通常，如果没有合适的继承类，就使用object类，这是所有类最终都会继承的类。
# 定义好了Student类，就可以根据Student类创建出Student的实例。创建实例是通过类名+()实现的：
# bart = Student()
# print(bart)    # ==> <__main__.Student object at 0x000001D0AF160830>
...
#
#     def print_score2(self, other_param):
#         print('%s: %s, other_param: %s' % (self.name, self.score, other_param))
#
# bart = Student('Bart Simpson', 59)
#
# # 要定义一个方法，除了第一个参数是self外，其他和普通函数一样。要调用一个方法，只需要在实例变量上直接调用，除了self不用传递，其他参数正常传入：
# bart.print_score() # ==> Bart Simpson: 59
# bart.print_score2('other_param_str') # ==> Bart Simpson: 59, other_param: other_param_str
#
...
# print(lisa.age) # ==> 报错：AttributeError: 'Student' object has no attribute 'age'

# bart实例可以新增变量age，新增后的bart实例变量名称就和lisa不一样了。
